# Массив A (64 элемента, 64-битные числа)
...
def xor512(A, B):
    if len(A) != 64 or len(B) != 64:
        raise ValueError("Оба массива должны иметь длину 64.")
    B = [int(b, 16) for b in B]  # Convert hex strings to integers
    xor_result = [a ^ b for a, b in zip(A, B)]
    return xor_result


def add_circle_mod2(A: List[int], B: List[int]) -> List[int]:
    if len(A) != 64 or len(B) != 64:
        raise ValueError("Оба массива должны иметь длину 64.")

    rez = [0] * 64  # Результирующий массив
    carry = 0  # Перенос
    ...
    for i in range(63, -1, -1):
        ...
        rez[i] = sum_value & 0xFF  # Оставляем только младшие 8 бит
        carry = sum_value >> 8  # Сохраняем перенос (старший байт)

    return rez


def s_trans(param: List[int]) -> List[int]:
    if len(param) != 64:
        raise ValueError("Массив `param` должен иметь длину 64.")
    if len(S) != 256:
        raise ValueError("Таблица замен `S` должна содержать 256 элементов.")

    # Создаём результирующий массив, заменяя каждый элемент через таблицу S
    res = [S[byte] for byte in param]
    return res


def p_trans(param: List[int]) -> List[int]:
    if len(param) != 64:
        raise ValueError("Массив `param` должен иметь длину 64.")
    if len(P) != 64:
        raise ValueError("Массив перестановки `P` должен иметь длину 64.")

    # Создаём результирующий массив, переставляя элементы param согласно индексам P
    res = [param[P[i]] for i in range(64)]
    return res

def l_trans(param, A):
    # Инициализация массива для хранения результата
    res = [0] * 8  # Массив для хранения 8 значений (64 бита каждый)
    X = 0
    flag = False

    # Процесс обработки данных в блоках по 8 байт
    for i in range(7, -1, -1):  # Идем по блокам из param
        # Собираем 64-битное число X из 8 байт
        for c in range(8):
            X += param[i * 8 + c]  # Добавляем байт в X
            if c < 7:
                X = X << 8  # Сдвигаем на 8 бит влево, чтобы подготовить к следующему байту

        # Применяем побитовый XOR для каждого бита X с массивом A
        for j in range(63, -1, -1):  # Проходим по каждому из 64 бит в X
            flag = (X >> j) & 1  # Извлекаем j-й бит из X
            if flag:  # Если бит равен 1, выполняем операцию XOR с соответствующим элементом A
                res[i] ^= A[63 - j]  # XOR с элементом из A

        X = 0  # Сбрасываем X для следующего блока

    # Массив для хранения финального результата
    ret = [0] * 64  # 64 байта результата
    for n_res in range(8):  # Перебираем блоки из res
        for byte in range(7, -1, -1):  # Перебираем байты в каждом блоке
            A_val = (res[n_res] >> (byte * 8))  # Извлекаем нужный байт из res
            B = A_val & 0xFF  # Маскируем, чтобы оставить только 8 младших бит
            ret[n_res * 8 + (7 - byte)] = B  # Записываем в ret с учетом порядка байтов

    return ret  # Возвращаем результат

def make_key(K, i, C):
    # 1. Применяем Xor512
    K = xor512(K, C[i])  # предполагается, что C - это список массивов из 64 байтов
    # 2. Применяем S_trans
    K = s_trans(K)
    # 3. Применяем P_trans
    K = p_trans(K)
    # 4. Применяем L_trans
    K = l_trans(K, A)  # A нужно передать как параметр, предположительно это глобальная переменная или параметр функции
    return K

def e_trans(K, m):
    # 1. Выполняем начальный XOR
    param = xor512(K, m)

    # 2. Повторяем 12 раз операции подстановки, перестановки и трансформации
    for i in range(12):
        param = s_trans(param)  # Подстановка
        param = p_trans(param)  # Перестановка
        param = l_trans(param, A)  # Дополнительная трансформация
        K = make_key(K, i, C)  # Обновление ключа
        param = xor512(param, K)  # XOR с обновленным ключом
...
def g_n(N, h, m):
    ...
    # 2. Применяем подстановку, перестановку и трансформацию
    K = s_trans(K)  # Подстановка
    K = p_trans(K)  # Перестановка
    K = l_trans(K, A)  # Дополнительная трансформация
...
def convertor_c_to_i(mess, Len):
    message = bytearray()  # Создаем пустой bytearray, чтобы сохранить результат
    for c in range(0, Len, 2):
        # Преобразуем два символа из строки mess в байт
        a = int(mess[c:c+2], 16)  # Преобразуем пару символов в шестнадцатеричное число
        message.append(a)  # Добавляем байт в результат
    return bytes(message)  # Возвращаем результат как неизменяемый объект bytes

def get_hash(message, hash_length):
    IV = [1] * 64 if hash_length == 256 else [0] * 64  # Инициализация IV
    N = [0] * 64
    E = [0] * 64
    len_message = len(message) * 8  # Длина сообщения в битах
    num_blocks = len_message // 512  # Количество блоков
    length_last_block = len_message % 512  # Длина последнего блока

    if length_last_block == 0:
        length_last_block = 512
    if length_last_block > 0:
        num_blocks += 1  # Увеличиваем количество блоков, если последний блок не полный

    h = IV  # Начальное значение хэша
    count = 1
    ...
    while count <= num_blocks:
        ...
        if count < num_blocks:
            for b in range(64):
                block[b] = message[len_message // 8 - count * 64 + b]
            N_512 = [0] * 64
            N_512[62] = (512 >> 8) & 0xFF
            N_512[63] = 512 & 0xFF
        else:
            # Дополнение последнего блока
            dobor = (512 - length_last_block) // 8
            for d in range(dobor - 1):
                block[d] = 0
            block[dobor - 1] = 1
            for b in range(length_last_block // 8):
                block[b + dobor] = message[b]
            N_512 = [0] * 64
            N_512[62] = (length_last_block >> 8) & 0xFF
            N_512[63] = length_last_block & 0xFF

        # Применяем функцию G_n
        h = g_n(N, h, block)
        N = add_circle_mod2(N, N_512)
        E = add_circle_mod2(E, block)
        count += 1
    ...
    if hash_length == 512:
        print("H E S H  5 1 2")
        for c in range(64):
            print(format(h[c], '02x'), end=" ")
            if (c + 1) % 32 == 0:
                print()
        print()
        return h
    else:
        h256 = h[:32]  # Берем первые 32 байта (256 бит)
        print("H E S H  256")
        for c in range(32):
            print(format(h256[c], '02x'), end=" ")
            if (c + 1) % 32 == 0:
                print()
        print()
        return h256
...
if __name__ == "__main__":
    ...
    data = "Example message for hashing"
    hash_value = get_hash(data, hash_length=256)
    print(hash_value)
